...
constexpr uint32_t SAMPLE_PERIOD_US = 10000;  // muestreo a 100 Hz para evitar aliasing (f_nyquist = 2*44Hz = 88 Hz en este caso)
...
void loop() {
  ...
  unsigned long dt = micros() - ultimo_tiempo;
  ultimo_tiempo += dt;
  
  angulo_gyro += gyro.gyro.x * dt * (180/PI);
  float angulo_acc = atan2(accel.acceleration.y, accel.acceleration.z) * (180/PI);
